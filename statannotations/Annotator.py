...
class Annotator:
    ...
    def new_plot(
        ...
        **plot_params,
        ...
        self._plotter: _Plotter = self._get_plotter(
            engine, ax, pairs, plot, data, x, y, hue, order, hue_order, **plot_params
        )
    ...
    def apply_and_annotate(self, hide_non_signif=False):
        ...
        * `hide_non_signif`: if True, do not annotate non-significant tests.
        ...
        if hide_non_signif:
            self.annotations = [
                annot for annot in self.annotations if annot.data.pvalue < self._alpha
            ]
    ...
    def apply_test(self, num_comparisons="auto", **stats_params):
        ...
        self.annotations = self._get_results(
            num_comparisons=num_comparisons, **stats_params
        )
        self._deactivate_configured_warning()

        return self

    def set_pvalues_and_annotate(
        self, pvalues, num_comparisons="auto", hide_non_signif=False
    ):
    ...
    def get_offset_func(cls, position):
        return {"inside": cls._get_offsets_inside, "outside": cls._get_offsets_outside}[
            position
        ]
    ...
    def _get_results(
        self, num_comparisons, pvalues=None, **stats_params
    ) -> List[Annotation]:
        test_result_list = []
        ...
        for group_struct1, group_struct2 in self._struct_pairs:
            ...
            if self.perform_stat_test:
                result = self._get_stat_result_from_test(
                    group_struct1, group_struct2, num_comparisons, **stats_params
                )
            else:
            ...
            test_result_list.append(
                Annotation(
                    (group_struct1, group_struct2), result, formatter=self.pvalue_format
                )
            )
    ...
    def _annotate_pair(self, annotation, ax_to_data, ann_list, orig_value_lim):
        ...
        xy_params = self._get_xy_params(group_coord_1, group_coord_2, line_x, line_y)
    ...
    def _check_test_pvalues_perform(self):
        ...
        if self.test not in IMPLEMENTED_TESTS and not isinstance(self.test, StatTest):
            raise ValueError(
    ...
    def _check_pvalues_no_perform(self, pvalues):
        ...
        if len(pvalues) != len(self.pairs):
            raise ValueError("`pvalues` should be of the same length as " "`pairs`.")
    ...
    def _check_correct_number_custom_annotations(self, text_annot_custom):
        ...
        if len(text_annot_custom) != len(self._struct_pairs):
            raise ValueError("`text_annot_custom` should be of same length as `pairs`.")
    ...
    def _get_stat_result_from_test(
        self, group_struct1, group_struct2, num_comparisons, **stats_params
    ) -> StatResult:
        result = apply_test(
    ...
    def validate_test_short_name(self):
        ...
        if self.show_test_name and self.pvalue_format.text_format != "star":
            if self.test:
                self.test_short_name = (
                    self.test if isinstance(self.test, str) else self.test.short_name
                )
    ...
    def has_type0_comparisons_correction(self):
        return (
            self.comparisons_correction is not None
    ...
    def _plot_line(self, line_x, line_y):
        if self.loc == "inside":
            self.ax.plot(
                line_x, line_y, lw=self.line_width, c=self.color, clip_on=False
            )
        else:
            line = lines.Line2D(
                line_x,
    ...
    def _annotate_pair_text(self, ann, value):
        ...
        if self.use_fixed_offset or got_mpl_error:
            if self._verbose >= 1:
                print(
                    "Warning: cannot get the text bounding box. Falling "
            ...
            offset_trans = mtransforms.offset_copy(
                trans=self.ax.transAxes, fig=self.fig, units="points", x=x, y=y
            )
            ...
            value_top_annot = self.ax.transAxes.inverted().transform(value_top_display)[
                value_coord
            ]
    ...
    def _warn_alpha_thresholds_if_necessary(parameters):
        if parameters.get("alpha"):
            ...
            if pvalue_format is None or pvalue_format.get("pvalue_thresholds") is None:
                warnings.warn(
                    "Changing alpha without updating "
    ...
    def _get_xy_params_horizontal(
        self, group_coord_1, group_coord_2, line_x: np.ndarray
    ):
        return {
    ...
    def _get_xy_params_vertical(self, group_coord_1, group_coord_2, line_y: np.ndarray):
        return {
    ...
    def _get_xy_params(
        self, group_coord_1, group_coord_2, line_x: np.ndarray, line_y: np.ndarray
    ):
        if self.orient == "h":
            return self._get_xy_params_horizontal(group_coord_1, group_coord_2, line_x)

        return self._get_xy_params_vertical(group_coord_1, group_coord_2, line_y)

    def _maybe_warn_about_configuration(self):
        if self._should_warn_about_configuration:
            warnings.warn(
                "Annotator was reconfigured without applying the "
